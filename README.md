...
* Concerning data: I now have access to both quark and gluon data of the same distributions (DAOD_JETM6). Both sets have passed the first processing step (Athena Algortihm) and are being translated into Junipr Jets. This takes some time given how large they are (processing the 916,786 gluon jets is expected to take close to 13 hours for example) and given (for some reason) HTCondor is far less efficient in data transfer than simply using the pplxint (problem is that the job is too long for pplxint11 that kills after a few hours). I will be running on this data as it has quark (with ttbar) and gluon (with dijet) samples of comparable information. In particular, it has similar sorts of jets and the same r and e tags. 
* A very annoying problem occurs with Condor. Sometimes, it does not return all of the files being produced by codes it runs. I had this issue when running a training loop for a junipr model and for the second step of data processing on the new files (it rendered 25 hours of processing useless). I contacted the IT department and it seems there was no mistake on my side of the submission but that there have been a "lot of network glitches recently, and a lot of jobs have been using too much memory and killing other processes on the worker nodes, sothis might be the cause" (according to Michael Leech of the IT team. He advised me to contact Vip who is said to know more about this). I am re-running the processing for dijet and ttbar now (for each, I running the job twice in parallel with HTCondor). 
...
* Next steps: 
    * Train a quark and a gluon models on the new similar data. 
    * Train these two models on the binary objective to get some discrimination performance.
    * Compare this to the benchmark models trained on the data used for binary objective.
    * Train a set of model on light versions of the data using different jet objects (PFlow, Truth, EMTopo) to check for differences.
    * Could explore using LSTM cells instead of regular RNN cells. More generally, optimise the structure (such as different branch network architecture).
    * Analyse information produced by Junipr network. Such as the probability distribution of branching, mother and end (and compare it to that of the simulations).
