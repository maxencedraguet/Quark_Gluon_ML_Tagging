...
class JuniprRunner(_BaseRunner):
    def __init__(self, config: Dict):
        ...
        if self.binary_junipr:
            print("Loading the unary models for binary-JUNIPR")
    ...
    def setup_dataloader(self, config: Dict)->None:
        ...
        if self.chunked_dataset_bool:
            print("Using chunked datasets, disabling shuffling of index inter-chunks (not intra)")
            self.shuffle_datasets = False

        bool_suffle_test = False
        if self.train_bool:
            self.train_dataloader = torch.utils.data.DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=self.shuffle_datasets, num_workers = self.num_workers)
            self.validation_dataloader = torch.utils.data.DataLoader(self.validation_dataset, batch_size=self.batch_size, shuffle=self.shuffle_datasets, num_workers = self.num_workers)
            print("Batch size used for the validation dataloader: ", self.batch_size)
            if self.binary_junipr:
                ...
                self.small_validation_dataloader = torch.utils.data.DataLoader(self.small_validation_dataset, batch_size=self.batch_size, shuffle=self.shuffle_datasets, num_workers = self.num_workers)
    ...
    def reset_dataloader(self, new_batchsize, epoch):
        ...
        self.train_dataloader = torch.utils.data.DataLoader(self.train_dataset, batch_size=new_batchsize, shuffle=self.shuffle_datasets, num_workers = self.num_workers)
    ...
    def setup_optimiser(self, binary_case = False):
        ...
        elif self.lr_scheduler == "1epochJS":  # same as in main JUNIPR paper but shorter (scalled down by a factor of five as suggested in the binary paper).
            self.lr_scheduler_bool = True
            self.lr_schedule = {"1": 0.01, "2": 0.001, "3": 0.0001, "4": 0.001, "5": 0.0001, "6": 0.00001}
        elif self.lr_scheduler == "1epochJS_EXTRA":  # same as in main JUNIPR paper but slightly longer (scalled down by a factor of five as suggested in the binary paper).
            self.lr_scheduler_bool = True
            self.lr_schedule = {"1": 0.01, "2": 0.001, "3": 0.0001, "4": 0.001, "5": 0.0001, "6": 0.00001, "7":  0.0001, "9": 0.00001}
        ...
        elif self.batch_scheduler == "junipr_unary_SHORT":
            self.batchsize_scheduler_bool = True
            self.batchsize_schedule = {"1": 10, "4": 100}
    ...
    def compute_batch_losses(self, batch_input, batch_output, test_bool = False, binary_target = False):
        ...
        The input arguments batch_input, batch_output are dictionnaries with the info required.
        ...
        # Next manage the different cases where the loss has to be produced following a certain fashion.
        #First special case is if test_bool is true. Second is when binary_target is true and the final one is for regular unary training
        
        if test_bool:
            # Returns the batch loss and a list of tuples (one for each event) with the loss of the event and the loss per node.
            # This is only really necessary when plotting a junipr tree and should be avoided when possible as it is quite costly.
            ...
            for sample_id in range(batch_size):     # forced to loop to restrict the sum to the right elements without too much I/O from dataloader.
            ...
            if binary_target:
                # Specific subcase, where the batch loss should be a tensor with an entry for each sample (which is already the case):
            ...
            # Returns the loss per event in a tensor batch_loss_tensor.
            batch_loss_tensor = torch.empty((batch_size, 1))
            # Combine and sum (note that in this case p_very_end_output_loss has to be managed seperately because it is of a different size.
            list_losses = [p_mother_output_loss, p_end_output_loss, p_branch_z_output_loss,
                           p_branch_t_output_loss, p_branch_p_output_loss, p_branch_d_output_loss]
            ...
            for sample_id in range(batch_size):     # forced to loop to restrict the sum to the right elements without too much I/O from dataloader.
                n_branching_in_sample = input_n_branchings[sample_id].item()
                value = losses_combined[running_branching_count:(running_branching_count+n_branching_in_sample)].sum() + p_very_end_output_loss[sample_id]
                batch_loss_tensor[sample_id] = value
                running_branching_count += n_branching_in_sample
            return batch_loss_tensor
        else:
            # Just returns the loss on the batch.
    ...
    def compute_losses(self, batch_input, batch_output, test_bool = False, binary_target = False):
        """
        To compute the loss in a sequential way. WARNING: : DEPRECATED. Better to use compute_batch_losses
        ...
        for batch_element in range(input_n_branchings.size()[0]):
            ...
            element_output_very_end   = output_very_end[batch_element, 0, :]
    ...
    def compute_batch_binary_losses(self, batch_input, batch_output_quark, batch_output_gluon, val_loss = False, test_loss = False, detail_loss = False):
        """
        To compute the binary loss in the batch-leaning environment (note that it is the MEAN loss computed here and it should thus NOT be averaged over batch size)
        
        The input arguments batch_input, batch_output are dictionnaries with the entries required.
        ...
        This implementation assumes equal likelihood of having a quark or a gluon jet (this has to be forced at dataloader level). It computes a sigmoid on log( Proba quark) - log (Proba gluon)
        
        Note that self.compute_batch_losses returns probabilities as -log(Proba) hence there are some sign flips in the processing here.
        """
        ...
        # Compute the unary losses using the compute_batch_losses (if detail_loss, you need to retrieve the list of probabilities per nodes
        if detail_loss:
        ...
        # Compute the real proba ratio (in log) and shape it appropriately.
        ...
        if self.binary_BCE_loss:
            ...
            batch_loss = torch.sigmoid(tensor_proba_ratio * sign_label.detach()).mean()  #correct the sign of the ratio depending on the label and applies a sigmoid as well as taking the mean.

        # To gauge accuracy
        tensor_sigmoid_ratio = torch.sigmoid(tensor_proba_ratio).detach()
    
        if test_loss:
            # Batches is of size 1. Here, you threshold the label in the test loop.
            ...
            if val_loss:
                accuracy_info = (correct_count / size_batch, tensor_sigmoid_ratio, true_label)
            else:
                accuracy_info = correct_count / size_batch

        if detail_loss:     # Need to further analyse the probability per node.
    ...
    def compute_binary_losses(self, batch_input, batch_output_quark, batch_output_gluon, val_loss = False, test_loss = False, detail_loss = False):
        """
        To compute the binary loss in a sequential way. WARNING: : DEPRECATED. Better to use compute_batch_binary_losses
        ...
        This implementation assumes equal likelihood of having a quark or a gluon jet (should be enforced at dataset level). It computes a sigmoid on log( Proba quark) - log (Proba gluon)
        
        Note that self.compute_losses returns probabilities as -log(Proba).
        """
        batch_loss = 0
        true_label = batch_input["label"]   # the MC label telling you if it is a quark (1) gluon (0). Note that -1 values should have been handled by the dataset
        size_batch = true_label.size()[0]
    ...
    def binary_train(self):
        """
        The binary JUNIPR trainer. Requires a model for quarks and another one for gluons.
        """
        # Set model to train mode
        ...
        for epoch in range(1 + self.previously_trained_number_epoch, self.num_epochs + 1 + self.previously_trained_number_epoch):
            ...
            for batch_input in self.train_dataloader:
                ...
                if (int(number_samples_processed_since_last) >= int(self.test_frequency * 200)):
                    ...
                    # Doing a test loop:
                    test_loss, test_acc, test_auc = self.binary_test_loop(step=step_count, use_small_val = True, batch = True)
                    #test_loss, test_acc, test_auc = self.binary_test_loop(step=step_count, use_small_val = True, batch = False)
    ...
    def binary_test_loop(self, step:int, use_small_val = False, batch = True, log_trees = False):
        """
        Perform a binary test loop on the validation dataset (or the smaller one, depending on use_small_val).
        batch indicates whether to use the batch loss or not (by default it is True).
        log_trees indicats whether to log some JUNIPR jet trees during training. It is very costly to train with this one so it is switched off by default.
        """
        ...
        with torch.no_grad():
            ...
            # Choose a validation dataloader
            if use_small_val:
            ...
            get_an_event = False
            if log_trees:
                size_sample = len(dataloader_to_use)
            ...
            for batch_count, batch_input in enumerate(dataloader_to_use):
                ...
                if get_an_event and batch_count == index:
    ...
    def test_loop(self, step:int, batch = True, log_trees = False):
        """
        Perform a unary test loop on the validation dataset.
        batch indicates whether to use the batch loss or not (by default it is True).
        log_trees indicats whether to log some JUNIPR jet trees during training. It is very costly to train with this one so it is switched off by default.
        """
