...
class JuniprRunner(_BaseRunner):
    def __init__(self, config: Dict):
        ...
        if self.binary_junipr:
            ...
            if self.assess:
                ...
                self.run(config, load_bool = True, print_jets_bool = True, binary_junipr_bool = True)
        else
            if self.load_bool:
                print("Loading a Junipr model")
                self.run(config, load_bool = True, print_jets_bool = True)
            if self.train_bool:
    ...
    def compute_losses(self, batch_input, batch_output, test_bool = False):
        """
    ...
    def compute_binary_losses(self, batch_input, batch_output_quark, batch_output_gluon, detail_loss = False):
        ...
        for indice in range(len(list_proba_quark)):
            ...
            if used_label == 1:
                proba = torch.sigmoid(proba_quark - proba_gluon) #the real probability: take a minus in front of the proba of self.compute_losses. However, we want to maximise the result of the sigmoid, meaning meaning - the sigmoid.
            else:
        ...
        return batch_loss, detail_list_proba
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for batch_input in self.train_dataloader:
                ...
                if (step_count% self.test_frequency == 0):
                    print("Training {} step | batch loss : {}".format(step_count, batch_loss))
    ...
    def binary_train(self):
        """
        A binary trainer. One for quark, other for gluon.
        """
        # Set model to train mode
        self.JUNIPR_model.train()
        step_count = 0
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for batch_input in self.train_dataloader:
                ...
                batch_loss, _ = self.compute_binary_losses(batch_input, batch_output_quark, batch_output_gluon)
    ...
    def binary_test_loop(self, step:int):
        ...
        with torch.no_grad():
            ...
            print("testing {} step | loss =  {}".format(step, float(mean_loss)))
    ...
    def run(self, config, train_bool = False, load_bool = False, print_jets_bool = False, save_model_bool = False, binary_junipr_bool = False):
        ...
        """
        if train_bool and !binary_junipr_bool:
